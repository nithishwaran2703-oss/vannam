...
function ensureStoreExists() {
  try {
    ...
    if (!fs.existsSync(DATA_FILE)) {
      ...
      // Sync missing keys from defaultStore
      ...
      // Ensure admin & teacher have proper roles
      if (existing.users) {
        const admin = existing.users.find(u => u.email === 'admin@vannam.edu');
        ...
        if (!teacher) {
          existing.users.push(defaultStore.users.find(u => u.email === 'teacher@vannam.edu'));
          modified = true;
        }
      }
      if (modified) {
