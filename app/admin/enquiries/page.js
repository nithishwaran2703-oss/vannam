...
export default function EnquiriesManager() {
  ...
  const fetchEnquiries = async () => {
    try {
      ...
      if (targetId && !selectedEnquiry) {
        const found = (data.enquiries || []).find((e) => e.id === targetId);
  ...
  const statusPills = [
    ...
    { label: 'New / Unread', value: 'new', count: stats.new || 0, color: 'bg-rose-100 text-rose-700' },
    { label: 'Contacted', value: 'contacted', count: stats.contacted || 0, color: 'bg-blue-100 text-blue-700' },
    { label: 'Follow-up', value: 'followup', count: stats.followup || 0, color: 'bg-amber-100 text-amber-700' },
    { label: 'Resolved', value: 'resolved', count: stats.resolved || 0, color: 'bg-emerald-100 text-emerald-700' },
    { label: 'Archived', value: 'archived', count: stats.archived || 0, color: 'bg-slate-100 text-slate-700' }
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-[#0F2963] tracking-tight flex items-center gap-2">
            <Mail className="w-6 h-6 text-[#00A8E8]" />
            <span>Parent Enquiries & Leads CRM</span>
          </h1>
          <p className="text-xs text-slate-500 font-medium">
            Review parent inquiries, track response timelines, manage follow-up notes, and convert leads into admissions.
          </p>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
          <form onSubmit={handleSearchSubmit} className="relative w-full sm:w-96">
            ...
            <input
              ...
              placeholder="Search by parent name, phone, program, keyword..."
      ...
      {/* Main Content Layout: Table (or empty state) + Detail Drawer */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        
        {/* Enquiries Table (Spans full width or 7 cols if item selected) */}
        <div className={`${selectedEnquiry ? 'lg:col-span-7' : 'lg:col-span-12'} bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden transition-all duration-300`}>
          <div className="p-4 sm:p-5 border-b border-slate-100 flex items-center justify-between">
            <h3 className="font-extrabold text-xs text-[#0F2963] uppercase tracking-wider">
              Inquiry Queue ({enquiries.length})
            </h3>
          ...
          {loading ? (
            ...
            <div className="p-12 text-center text-slate-400 text-xs">
              No inquiries found in this filter view.
            </div>
          ) : (
                    ...
                    return (
                      <tr
                        ...
                        className={`cursor-pointer transition ${
                          isSelected
                            ? 'bg-[#E8EEFB]/70 border-l-4 border-l-[#00A8E8]'
                      ...
                      >
                        ...
                        <td className="py-3.5 px-4">
                          <div className="font-extrabold text-[#0F2963] text-xs leading-snug">
                            {enq.parentName}
                          </div>
                        ...
                        <td className="py-3.5 px-4">
                          <div className="font-bold text-xs text-[#0F2963]">{enq.program}</div>
                          {enq.childAge && (
                            <div className="text-[11px] text-[#00A8E8]">Age: {enq.childAge}</div>
                          )}
                        </td>
                        ...
                        <td className="py-3.5 px-4 text-right">
                          <span className="text-xs font-bold text-[#00A8E8] inline-flex items-center gap-0.5">
                            <span>Inspect</span>
        ...
        {/* Right: Enquiry Detail Inspection Drawer (5 cols) */}
        {selectedEnquiry && (
          <div className="lg:col-span-5 bg-white rounded-3xl border border-slate-100 shadow-xl p-6 space-y-6 self-start sticky top-20 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="flex items-start justify-between pb-3 border-b border-slate-100">
              <div>
                ...
                <h3 className="font-black text-lg text-[#0F2963] leading-tight">
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {['new', 'contacted', 'followup', 'resolved', 'archived'].map((st) => (
                  <button
                    key={st}
                    type="button"
            ...
            <div className="space-y-1 p-4 rounded-2xl bg-slate-50 border border-slate-100">
              <div className="text-[10px] font-bold uppercase text-slate-400">Parent Inquiry Message</div>
              <p className="text-xs text-slate-700 leading-relaxed italic">
                &quot;{selectedEnquiry.message || 'No specific text written.'}&quot;
              </p>
              <div className="pt-2 text-[11px] font-semibold text-[#00A8E8]">
                Program: {selectedEnquiry.program} | Child Age: {selectedEnquiry.childAge || 'Not specified'}
              </div>
            </div>

            {/* Internal Staff Notes Logger */}
            <div className="space-y-3">
              ...
              <form onSubmit={handleAddNote} className="flex gap-2">
                <input
                  ...
                  placeholder="Log call outcome or follow up date..."
                ...
                {(selectedEnquiry.notes || []).map((note, idx) => (
                  <div key={idx} className="p-3 rounded-xl bg-slate-50 border border-slate-100 text-xs space-y-1">
                    <div className="flex items-center justify-between text-[10px] text-slate-400">
                      ...
                      <span>{new Date(note.date).toLocaleDateString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white max-w-sm w-full rounded-3xl p-6 space-y-4 shadow-2xl">
            ...
            <div className="text-center space-y-1">
              <h3 className="font-extrabold text-slate-800 text-base">Delete Enquiry?</h3>
              <p className="text-xs text-slate-500">
                This will permanently delete this lead record and its logged notes.
              </p>
            </div>
